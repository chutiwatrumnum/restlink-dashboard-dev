...
// === UPLOAD JURISTIC IMAGE ===
export const useUploadJuristicImageMutation = () => {
  return useMutation({
    retry: 1,
    scope: {
      id: "uploadJuristicImage",
    },
    mutationFn: async (base64String: string) => {
      try {
        ...
        const response = await axios.put(`/users/juristic/image-profile`, payload, {
          headers: {
        ...
        return response.data.imageUrl || response.data.data?.imageUrl || response.data.url || "success";

      } catch (error: any) {
        console.error("Image upload error:", error);

        if (error.response) {
          const errorMessage = error.response.data?.message || `Upload failed: ${error.response.status}`;
          throw new Error(errorMessage);
        }
...
// === UTILITY FUNCTION (ไม่ใช่ mutation) ===
