    ...
    return (
        <div className="!w-full !h-full">
            {deviceDisplay.length === 0 && (
                <div className="!w-full !h-full p-4 md:p-6 lg:p-0 bg-white">
                    <div className="flex justify-center items-center w-full h-full rounded-2xl shadow-lg overflow-hidden">
                        ...
                        {deviceDisplay.map((device: any, indexParent: number) => (
                                <div key={device?.id || indexParent} className={`${indexParent !== deviceDisplay.length - 1 ? 'mb-4' : ''} bg-white rounded-2xl overflow-hidden shadow`}>
                                     ...
                                     <div className="p-3 sm:p-4 md:p-5 lg:p-6 h-full ">
                                         ...
                                         <div className="flex items-center justify-start gap-2 mb-2 sm:mb-3 md:mb-4 lg:mb-6">
                                             ...
                                             <span className=" font-semibold text-gray-900 ">
                                                 <div className="flex flex-wrap gap-2">
                                                     {
                                                         device.events.map((event: any,indexDevice: number) => (
                                                             <div key={indexDevice} className="flex flex-wrap gap-2">
                                                                 {event || '-'}
                                                             </div>
                                                         ))
                                                     }
                                         ...
                                         {/* Device Info */}
                                         {
                                            (device?.properties || []).flatMap((property: any, propIndex: number) => 
                                                (property || []).map((item: any, index: number) => (
                                                    <div key={`prop-${propIndex}-${index}`} className="flex items-center justify-between text-sm rounded-lg p-2 sm:p-3 md:p-4 
                                                mt-2 sm:mt-3 md:mt-0 lg:mt-0">
                                                        <div className="flex items-center gap-2">
                                                            <img src={Card} alt="Card" className="scale-75" />
                                                            <span className="text-gray-600  font-semibold  ">{item?.ip || '-'}</span>
                                                        </div>
                                                        <div className="flex items-center gap-2">
                                                            <img src={Computer} alt="Computer" className="scale-75" />
                                                            <span className="text-gray-600 font-semibold ">{device?.custom_name || '-'}</span>
                                                        </div>
                                                        <div className="flex items-center gap-2">
                                                            <img src={Battery} alt="Battery" className="scale-75" />
                                                            <span className="text-green-600 font-semibold ">{item?.value || '0'}%</span>
                                                        </div>
                                                    </div>
                                                ))
                                            )
